...
if p_lock then
	// ロック
	if lck.of_gtin_kanri_lock( i_pg_id, i_jigyosya_code ) then
	...
	// アンロック
	if lck.of_gtin_kanri_unlock( i_pg_id, i_jigyosya_code ) then
		commit using cocos;  // commitは子孫側で行う
	else
		return false
	end if
...
// 複合チェック
// 自動採番「1」
if dw_entry.object.jido_saiban_flg[1] = 1 then
	// 必須チェック
	if len( string( dw_entry.object.item_code_str[1] ) ) = 0 or isnull( dw_entry.object.item_code_str[1] ) then
	...
	if len( string( dw_entry.object.item_code_end[1] ) ) = 0 or isnull( dw_entry.object.item_code_end[1] ) then
	...
	if len( string( dw_entry.object.item_code_gen[1] ) ) = 0 or isnull( dw_entry.object.item_code_gen[1] ) then
	...
	// 大小チェック
	if dw_entry.object.item_code_str[1] > dw_entry.object.item_code_end[1] then
	...
	// 範囲チェック
	if dw_entry.object.item_code_str[1] > dw_entry.object.item_code_gen[1] or dw_entry.object.item_code_end[1] < dw_entry.object.item_code_gen[1] then
