...
if p_lock then
	// ロック
	if lck.of_tanto_lock( i_pg_id, i_tanto_code ) then
	...
	// アンロック
	if lck.of_tanto_unlock( i_pg_id, i_tanto_code ) then
		commit using cocos;  // commitは子孫側で行う
	else
		return false
	end if
		...
		if lena( trim( data ) ) > 0 then
			...
			if i_tanto_code <> l_code then
				...
				choose case cocos.sqlcode
					...
					case 100
						msg.of_error_dwo( dwo, fnc.strg.of_format( "入力された{1}はマスタに存在しません。", l_colnm ) )
						return 1
					case else
						msg.of_error_dwo( dwo, fnc.strg.of_format( "{1}入力チェック中にエラーが発生しました{2}（{3}）", l_colnm, code.crlf, fnc.strg.of_replace( cocos.sqlerrtext, code.lf, " " ) ) )
						return 1
				end choose
			end if
		end if
