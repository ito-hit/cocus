$PBExportHeader$wld_tanka.srw
$PBExportComments$郵便番号アップロード登録
forward
global type wld_tanka from iw_main_window
end type
type st_status from ivo_statictext within wld_tanka
end type
type st_msg from ivo_statictext within wld_tanka
end type
type cb_clear from ivo_commandbutton within wld_tanka
end type
type dw_list from ivo_datawindow within wld_tanka
end type
type st_filename_t from ivo_statictext within wld_tanka
end type
type st_filename from ivo_statictext within wld_tanka
end type
type st_cnt_t from ivo_statictext within wld_tanka
end type
type st_cnt from ivo_statictext within wld_tanka
end type
type st_okcnt_t from ivo_statictext within wld_tanka
end type
type st_okcnt from ivo_statictext within wld_tanka
end type
type st_errcnt_t from ivo_statictext within wld_tanka
end type
type st_errcnt from ivo_statictext within wld_tanka
end type
type st_warncnt_t from ivo_statictext within wld_tanka
end type
type st_warncnt from ivo_statictext within wld_tanka
end type
type ddplb_status from ivo_dropdownpicturelistbox within wld_tanka
end type
type ddplb_msg from ivo_dropdownpicturelistbox within wld_tanka
end type
type hpb_progres from ivo_hprogressbar within wld_tanka
end type
end forward

global type wld_tanka from iw_main_window
string title = "単価アップロード登録"
boolean f7_key = true
...
decimal	l_tanka
decimal l_jyoudai,l_sin_jyoudai

long	l_up_iti,l_up_gaku
long	l_up_1,l_up_2,l_up_3,l_up_4,l_up_5
string	l_zei_kbn
decimal	l_zeiritu
long	l_tekiyo_date_str,l_tekiyo_date_end
long	l_sin_jyoudai_tekiyou_date
...
declare cur_syohin cursor for
	select	m.hin_ban,m.hin_mei,
			m.kakaku_group_kbn,
			m.jyoudai,
			m.up_iti,max(m.up_gaku) up_gaku,
			m.sin_jyoudai,m.sin_jyoudai_tekiyou_date,
			m.haiban_kbn,m.syusoku_flg
	  from	m_syohin m,
			m_meisyo mm
	 where	m.haiban_kbn	= mm.meisyo_code(+)
	   and	122				= mm.meisyo_kbn(+)
	   and	m.syohin_code	= :l_syohin_code
	 group by m.hin_ban,m.hin_mei,
	 		m.kakaku_group_kbn,m.jyoudai,m.up_iti,
			m.sin_jyoudai,m.sin_jyoudai_tekiyou_date,
			m.haiban_kbn,m.syusoku_flg
	using cocos;
...
dwobject dwlo_x_yakujyo
dwobject dwlo_s_jyoudai
dwobject dwlo_s_kakeritu
dwobject dwlo_s_kakaku_group
dwobject dwlo_s_up_tanka
dwobject dwlo_s_up1
dwobject dwlo_s_up2
dwobject dwlo_s_up3
dwobject dwlo_s_up4
dwobject dwlo_s_up5
dwobject dwlo_s_sin_jyoudai
dwobject dwlo_s_sin_jyoudai_tekiyou_date
dwobject dwlo_s_haiban_kbn
dwobject dwlo_t_syusoku_flg
dwobject dwlo_s_syusoku_flg
...
//				end if
	...
	dwlo_x_operation                = dw_list.object.x_operation
	dwlo_m_tanka                    = dw_list.object.m_tanka
	dwlo_m_kakeritu                 = dw_list.object.m_kakeritu
	dwlo_m_up1                      = dw_list.object.m_up1
	dwlo_m_up2                      = dw_list.object.m_up2
	dwlo_m_up3                      = dw_list.object.m_up3
	dwlo_m_up4                      = dw_list.object.m_up4
	dwlo_m_up5                      = dw_list.object.m_up5
	dwlo_m_zei_kbn                  = dw_list.object.m_zei_kbn
	...
	dwlo_x_yakujyo                  = dw_list.object.x_yakujyo
	dwlo_s_jyoudai                  = dw_list.object.s_jyoudai
	dwlo_s_kakeritu                 = dw_list.object.s_kakeritu
	dwlo_s_kakaku_group             = dw_list.object.s_kakaku_group
	dwlo_s_up_tanka                 = dw_list.object.s_up_tanka
	dwlo_s_up1                      = dw_list.object.s_up1
	dwlo_s_up2                      = dw_list.object.s_up2
	dwlo_s_up3                      = dw_list.object.s_up3
	dwlo_s_up4                      = dw_list.object.s_up4
	dwlo_s_up5                      = dw_list.object.s_up5
	dwlo_s_sin_jyoudai              = dw_list.object.s_sin_jyoudai
	dwlo_s_sin_jyoudai_tekiyou_date = dw_list.object.s_sin_jyoudai_tekiyou_date
	dwlo_s_haiban_kbn               = dw_list.object.s_haiban_kbn
	dwlo_t_syusoku_flg              = dw_list.object.t_syusoku_flg
	dwlo_s_syusoku_flg              = dw_list.object.s_syusoku_flg
	...
	for l_cnt = i_str_fileread.ret[1].startrow to i_dts.rowcount()
		...
		l_syohin_code	= long(dso_data02.primary[l_cnt])	//商品コード
		
		// 1.取込ファイル項目設定
		dwlo_f_tokui_code.primary[l_row]				= long(dso_data01.primary[l_cnt])	//得意先コード
		dwlo_f_syohin_code.primary[l_row]				= long(dso_data02.primary[l_cnt])	//商品コード
		dwlo_f_tanka.primary[l_row]						= dec(dso_data03.primary[l_cnt])	//下代
		dwlo_f_up1.primary[l_row]						= dec(dso_data04.primary[l_cnt])	//Up1
		dwlo_f_up2.primary[l_row]						= dec(dso_data05.primary[l_cnt])	//Up2
		dwlo_f_up3.primary[l_row]						= dec(dso_data06.primary[l_cnt])	//Up3
		dwlo_f_up4.primary[l_row]						= dec(dso_data07.primary[l_cnt])	//Up4
		dwlo_f_up5.primary[l_row]						= dec(dso_data08.primary[l_cnt])	//Up5
		dwlo_f_zei_kbn.primary[l_row]					= int(dso_data09.primary[l_cnt])	//税区分
		dwlo_f_zei_ritu.primary[l_row]					= dec(dso_data10.primary[l_cnt])	//税率
		dwlo_f_tekiyo_date_sta.primary[l_row]			= long(dso_data11.primary[l_cnt])	//適用開始日
		dwlo_f_tekiyo_date_end.primary[l_row]			= long(dso_data12.primary[l_cnt])	//適用終了日
		...
		dwlo_m_tanka.primary[l_row]						= l_tanka	//下代
		dwlo_m_up1.primary[l_row]						= l_up_1	//Up1
		dwlo_m_up2.primary[l_row]						= l_up_2	//Up2
		dwlo_m_up3.primary[l_row]						= l_up_3	//Up3
		dwlo_m_up4.primary[l_row]						= l_up_4	//Up4
		dwlo_m_up5.primary[l_row]						= l_up_5	//Up5(別寸)
		dwlo_m_zei_kbn.primary[l_row]					= l_zei_kbn	//税区分
		dwlo_m_zei_ritu.primary[l_row]					= l_zeiritu	//税率
		...
		fetch next cur_syohin
		 into :l_hin_ban,:l_hin_mei,
		 	  :l_kakaku_group_kbn,
		 	  :l_jyoudai,
		 	  :l_up_iti,:l_up_gaku,
		 	  :l_sin_jyoudai,:l_sin_jyoudai_tekiyou_date,
		 	  :l_haiban_kbn,
		 	  :l_syusoku_flg;
		
		dwlo_s_hin_ban.primary[l_row]					= l_hin_ban	//品番
		dwlo_s_hin_mei.primary[l_row]					= l_hin_mei	//品名
		dwlo_s_kakaku_group.primary[l_row]				= l_kakaku_group_kbn	//価格グループ区分
		dwlo_s_jyoudai.primary[l_row]					= l_jyoudai	//上代
		dwlo_s_sin_jyoudai.primary[l_row]				= l_sin_jyoudai	//新上代
		dwlo_s_sin_jyoudai_tekiyou_date.primary[l_row]	= l_sin_jyoudai_tekiyou_date	//新上代適用開始日
		dwlo_s_haiban_kbn.primary[l_row]				= l_haiban_kbn	//廃番区分
		dwlo_s_syusoku_flg.primary[l_row]				= l_syusoku_flg	//終息フラグ(商品)
		
		dwlo_s_up1.primary[l_row]						= 0
		dwlo_s_up2.primary[l_row]						= 0
		dwlo_s_up3.primary[l_row]						= 0
		dwlo_s_up4.primary[l_row]						= 0
		dwlo_s_up5.primary[l_row]						= 0
		
		do while cocos.sqlcode = 0
			// アップ位置/アップ額
			choose case l_up_iti
				case 1
					dwlo_s_up1.primary[l_row]			= l_up_gaku	//Up1
					
				case 2
					dwlo_s_up2.primary[l_row]			= l_up_gaku	//Up2
					
				case 3
					dwlo_s_up3.primary[l_row]			= l_up_gaku	//Up3
					
				case 4
					dwlo_s_up4.primary[l_row]			= l_up_gaku	//Up4
					
				case 5
					dwlo_s_up5.primary[l_row]			= l_up_gaku	//Up5(別寸)
		...
		dwlo_s_kakeritu.primary[l_row]					= l_kakeritu	//掛率
		...
		// 6.算出項目設定
		if dwlo_f_tanka.primary[l_row] = 0 then
		...
		dwlo_x_yakujyo.primary[l_row]					= trim(l_tokui_mei)	//約定単価
		
		if l_upkomi_kbn >= 1 then dwlo_f_up1.primary[l_row] = 0	//Up1
		if l_upkomi_kbn >= 2 then dwlo_f_up2.primary[l_row] = 0	//Up2
		if l_upkomi_kbn >= 3 then dwlo_f_up3.primary[l_row] = 0	//Up3
		if l_upkomi_kbn >= 4 then dwlo_f_up4.primary[l_row] = 0	//Up4
		if l_upkomi_kbn >= 5 then dwlo_f_up5.primary[l_row] = 0	//Up5
...
type uo_signal from iw_main_window`uo_signal within wld_tanka
end type

type st_date from iw_main_window`st_date within wld_tanka
end type

type st_user_mei from iw_main_window`st_user_mei within wld_tanka
end type

type uo_test_mode from iw_main_window`uo_test_mode within wld_tanka
end type

type p_size_adjust from iw_main_window`p_size_adjust within wld_tanka
end type

type st_title from iw_main_window`st_title within wld_tanka
end type

type st_title_upper from iw_main_window`st_title_upper within wld_tanka
end type

type st_upper from iw_main_window`st_upper within wld_tanka
end type

type st_second_upper from iw_main_window`st_second_upper within wld_tanka
boolean f6_key = true
boolean f7_key = true
boolean f9_key = true
...
type cb_retrieve from iw_main_window`cb_retrieve within wld_tanka
end type
