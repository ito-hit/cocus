$PBExportHeader$wmm_tanka.srw
$PBExportComments$単価登録(単品)
forward
global type wmm_tanka from iw_main_window
end type
type st_syori_kbn from ivo_statictext within wmm_tanka
end type
type ddplb_syori_kbn from sv_ddplb_syori within wmm_tanka
end type
type st_kyoten_code from ivo_statictext within wmm_tanka
end type
type sle_kyoten_code from ivo_singlelineedit within wmm_tanka
end type
type cb_kyoten_code from sv_cb_modal within wmm_tanka
end type
type st_kyoten_mei from sv_st_meisyo within wmm_tanka
end type
type dw_entry from ivo_datawindow within wmm_tanka
end type
type uo_tokuisaki from cv_input_tokui within wmm_tanka
end type
end forward

global type wmm_tanka from iw_main_window
string title = "単価登録(単品)"
			...
			select	m.rowid,m.haita_flg,
			  ...
			  from m_tanka m,
				 ...
				 where m.kousin_user_id	= mu.user_id(+)
				   ...
				   and m.syohin_code	= :sle_kyoten_code.text
			   for update
			using cocos;
	...
	case false
		...
		update m_tanka
		   set haita_flg		= 0,
			   ...
			   kousin_client_id	= :device.name,
			   kousin_pg_id		= :i_pg_id
		 where syohin_code		= :sle_kyoten_code.text
		   and haita_flg		= 1
		 using cocos;
...
this.Control[iCurrent+8]=this.uo_tokuisaki
end on

on wmm_tanka.destroy
call super::destroy
...
type uo_signal from iw_main_window`uo_signal within wmm_tanka
end type

type st_date from iw_main_window`st_date within wmm_tanka
end type

type st_user_mei from iw_main_window`st_user_mei within wmm_tanka
end type

type uo_test_mode from iw_main_window`uo_test_mode within wmm_tanka
end type

type p_size_adjust from iw_main_window`p_size_adjust within wmm_tanka
end type

type st_title from iw_main_window`st_title within wmm_tanka
end type

type st_title_upper from iw_main_window`st_title_upper within wmm_tanka
end type

type st_upper from iw_main_window`st_upper within wmm_tanka
end type

type st_second_upper from iw_main_window`st_second_upper within wmm_tanka
integer y = 114
integer height = 370
end type

type cb_retrieve from iw_main_window`cb_retrieve within wmm_tanka
integer y = 373
integer height = 82
integer taborder = 40
end type
