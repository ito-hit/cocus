$PBExportHeader$wmm_unso.srw
$PBExportComments$運送便登録
forward
global type wmm_unso from iw_main_window
end type
type st_syori_kbn from ivo_statictext within wmm_unso
end type
type dw_list from ivo_datawindow within wmm_unso
end type
type ddplb_syori_kbn from sv_ddplb_syori within wmm_unso
end type
type dw_entry from ivo_datawindow within wmm_unso
end type
end forward
...
forward prototypes
private function boolean of_record_lock (string p_unso_code, boolean p_lock)
end prototypes
...
dw_list.height	= newheight - dw_entry.y - dw_list.y - code.MGN_TATE - key_fnc.height	//ReSize:(新高さ)-入力部y-自y(=固定部高さ)-縦マージン-ファンクション高さ
...
choose case ddplb_syori_kbn.text
	case "登録","変更","削除"
		...
		if not dw_entry.update(true,false) = 1 then
			msg.of_error(fnc.strg.of_format("運送便登録の{2}処理でエラーが発生しました。{1}{3}：{4}",code.crlf,ddplb_syori_kbn.text,string(cocos.last_sqlerrcode),cocos.last_sqlerrtext))
...
if uo_signal.of_get_status() then
	// 排他ロック解放する
	string l_unso_code
	if not of_record_lock(l_unso_code,false) then
		// ロック解放の成否は問わない
		//return 1
	end if
end if
...
if cocos.sqlcode <> 0 then
	// 通常エラーにはならない。想定外エラー
	msg.of_error(fnc.strg.of_format("運送便登録の検索処理でエラーが発生しました。{1}{2}：{3}",code.crlf,string(cocos.last_sqlerrcode),cocos.last_sqlerrtext))
...
type uo_signal from iw_main_window`uo_signal within wmm_unso
end type

type st_date from iw_main_window`st_date within wmm_unso
end type

type st_user_mei from iw_main_window`st_user_mei within wmm_unso
end type

type uo_test_mode from iw_main_window`uo_test_mode within wmm_unso
end type

type p_size_adjust from iw_main_window`p_size_adjust within wmm_unso
end type

type st_title from iw_main_window`st_title within wmm_unso
integer x = 2340
integer width = 440
end type

type st_title_upper from iw_main_window`st_title_upper within wmm_unso
end type

type st_upper from iw_main_window`st_upper within wmm_unso
integer y = 4
end type

type st_second_upper from iw_main_window`st_second_upper within wmm_unso
integer y = 139
boolean f9_key = true
boolean f10_key = true
end type

type cb_retrieve from iw_main_window`cb_retrieve within wmm_unso
integer taborder = 20
boolean f9_key = true
boolean f10_key = true
end type
...
integer taborder = 30
boolean bringtotop = true
string dataobject = "dmm_unso_list"
...
integer taborder = 30
boolean bringtotop = true
string dataobject = "dmm_unso_entry"
boolean hscrollbar = true
boolean vscrollbar = true
...
choose case dwo.name
	case "unso_code","unso_mei","unso_mei_ryaku","keisan_code","okurijyo_code","tyakubarai_unso_code"
		l_input	= trim(data)
		l_colnm	= dwo.tag		// タグ値で項目名を取得
		if not fnc.of_chk_column_info(l_input,false,l_colnm,'m_unso',dwo.name,l_msg) then
...
choose case dwo.name
	case "untin_flg"
		if data = "1" then
			dw_entry.object.keisan_code.enabled = "yes"
		else
			dw_entry.object.keisan_code.enabled = "no"
		end if
		
	case "okurijyo_flg"
		if data = "1" then
			dw_entry.object.okurijyo_code.enabled = "yes"
		else
			dw_entry.object.okurijyo_code.enabled = "no"
		end if
		
	case "tyakubarai_furikae_flg"
		if data = "1" then
			dw_entry.object.yakubarai_unso_code.enabled = "yes"
		else
			dw_entry.object.yakubarai_unso_code.enabled = "no"
		end if
...
string l_unso_mei
choose case dwo.name
	case "keisan_code","okurijyo_code","tyakubarai_unso_code"
		select unso_mei
		  into :l_unso_mei
		  from m_unso
		 where unso_code = :data
		 using cocos;
		 
		choose case dwo.name
			case "keisan_code"
				this.object.keisan_unso_bin_mei[row] = l_unso_mei
				
			case "okurijyo_code"
				this.object.okurijyo_unso_bin_mei[row] = l_unso_mei
				
			case "tyakubarai_unso_code"
				this.object.tyakubarai_unso_bin_mei[row] = l_unso_mei
