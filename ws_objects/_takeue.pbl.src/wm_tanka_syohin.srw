...
if sle_tokuisaki_code_from.text = "" then
	tokui_from = 1
else
	tokui_from = long(sle_tokuisaki_code_from.text)
end if
if sle_tokuisaki_code_to.text = "" then
	tokui_to = 99999
else
	tokui_to = long(sle_tokuisaki_code_to.text)
end if
...
declare cur_haita cursor for
	...
	and tokui_code	between :tokui_from and :tokui_to
...
// 取引停止先
string ls_torihiki_teisi
if rb_torihiki_teisi_kbn_yes.checked then
    ls_torihiki_teisi = rb_torihiki_teisi_kbn_yes.text
else
    ls_torihiki_teisi = rb_torihiki_teisi_kbn_no.text
end if

// 対象部門
string ls_taisyo_bumon
long l_count, l_i
...
for l_i = 1 to l_count
    if dw_list.object.chkbox[l_i] = "1" then
        ...
        ls_taisyo_bumon += dw_list.object.meisyo_1[l_i]
    end if
next

ls_message = st_title.text + "[商品=" + &
             string(uo_syohin.sle_input.text) + " " + &
             trim(string(uo_syohin.st_meisyo.text)) + " " + &				 
				 trim(string(uo_syohin.st_meisyo_hin_mei.text)) + " " + &
				 "対象部門=" + &
				 ls_taisyo_bumon + " " + &
				 "得意先コード=" + &
             trim(string(tokui_from)) + " ～ " + &
             trim(string(tokui_to)) + " " + &
				 "取引停止=" + ls_torihiki_teisi + " " + &
				 "上書き=" + ls_uwagaki + " "
...
dw_tanka.dataobject 	= "dm_tanka_syohin_tanka"
...
integer taborder = 20
boolean bringtotop = true
string dataobject = "dm_tanka_syohin_bumon"
