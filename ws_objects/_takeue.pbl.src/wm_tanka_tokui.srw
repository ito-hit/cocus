...
if sle_syohin_code_from.text = "" then
	syohin_from = 1
else
	syohin_from = long(sle_syohin_code_from.text)
end if
if sle_syohin_code_to.text = "" then
	syohin_to = 99999
else
	syohin_to = long(sle_syohin_code_to.text)
end if

declare cur_haita cursor for
	select	nvl(kousin_user_id,'???'),
			nvl(kousin_client_id,'???'),
			haita_flg
	  from    m_tanka 
	 where   tokui_code	= :l_tokui_code
	and syohin_code	between :syohin_from and :syohin_to
...
if cbx_tankahyo_9.checked then
    ls_tankahyo = cbx_tankahyo_9.text
end if
if cbx_tankahyo_1.checked then
    if len(ls_tankahyo) > 0 then ls_tankahyo += "&"
    ls_tankahyo += cbx_tankahyo_1.text
end if
if cbx_tankahyo_2.checked then
    if len(ls_tankahyo) > 0 then ls_tankahyo += "&"
    ls_tankahyo += cbx_tankahyo_2.text
end if
if cbx_tankahyo_3.checked then
    if len(ls_tankahyo) > 0 then ls_tankahyo += "&"
    ls_tankahyo += cbx_tankahyo_3.text
end if
...
ls_message = st_title.text + "[得意先=" + &
             string(uo_tokui.sle_input.text) + " " + &
             trim(string(uo_tokui.st_meisyo.text)) + " " + &
				 "対象商品=" + &
				 ls_taisyo_syohin + " " + &
				 "商品コード=" + &
             trim(string(syohin_from)) + " ～ " + &
             trim(string(syohin_to)) + " " + &
				 "単価表発行=" + ls_tankahyo + " " + &
				 "廃番区分=" + ls_haiban + " " + &
				 "上書き=" + ls_uwagaki + " "

ls_message += "]" + i_syori_kbn
...
// 単価表のチェックが一つも入っていなかったら
if cbx_tankahyo_9.checked = false and cbx_tankahyo_1.checked = false and cbx_tankahyo_2.checked = false and cbx_tankahyo_3.checked = false then
	msg.of_error( "単価表発行区分は必須入力です。" )
	return -1
end if
...
dw_tanka.dataobject 	= "dm_tanka_tokui_tanka"
...
integer taborder = 30
boolean bringtotop = true
string dataobject = "dm_tanka_tokui_kakeritu"
