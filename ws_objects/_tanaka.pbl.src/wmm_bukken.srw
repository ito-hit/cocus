...
// ログ出力
of_add_db_log( "of_db_delete", 0, st_title.text + "[" + string(p_rec.bukken_code) + " " + p_rec.bukken_mei + "]" + i_syori_kbn )
...
// ログ出力
of_add_db_log( "of_db_update", 0, st_title.text + "[" + string(p_rec.bukken_code) + " " + p_rec.bukken_mei + "]" + i_syori_kbn )
...
// ログ出力
of_add_db_log( "of_db_insert", 0, st_title.text + "[" + string(p_rec.bukken_code) + " " + p_rec.bukken_mei + "]" + i_syori_kbn )
...
if p_lock then
	// ロック
	if lck.of_bukken_lock( i_pg_id, i_bukken_code ) then
	...
	// アンロック
	if lck.of_bukken_unlock( i_pg_id, i_bukken_code ) then
		commit using cocos;  // commitは子孫側で行う
	else
		return false
	end if
...
l_file	= fnc.strg.of_format( "物件登録_{1}_{2}", string( today(), "yyyymmdd" ), string( now(), "hhmmss" ) )
...
choose case dwo.name
	case "bukken_code", "bukken_mei", "tokui_code"
		l_input	= trim( data )
