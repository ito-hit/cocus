...
// ログ出力
of_add_db_log( "of_db_delete", 0, st_title.text + "[" + string(p_rec.group_code) + " " + p_rec.group_mei + "]" + i_syori_kbn )
...
// ログ出力
of_add_db_log( "of_db_update", 0, st_title.text + "[" + string(p_rec.group_code) + " " + p_rec.group_mei + "]" + i_syori_kbn )
...
// ログ出力
of_add_db_log( "of_db_insert", 0, st_title.text + "[" + string(p_rec.group_code) + " " + p_rec.group_mei + "]" + i_syori_kbn )
...
choose case cocos.sqlcode
	case 0
		msg.of_error_dwo( p_dwo, fnc.strg.of_format( "登録しようとしているグループコードはすでに登録があります。{1}【登録済のグループ】{2}：{3}", code.crlf, p_group_code, l_db_group_mei ) )
		return false
	case 100
		return true
	case else
...
if p_lock then
	// ロック
	if lck.of_group_lock( i_pg_id, i_group_code ) then
	...
	// アンロック
	if lck.of_group_unlock( i_pg_id, i_group_code ) then
		commit using cocos;  // commitは子孫側で行う
	else
		return false
	end if
...
l_file	= fnc.strg.of_format( "商品グループ登録_{1}_{2}", string( today(), "yyyymmdd" ), string( now(), "hhmmss" ) )
