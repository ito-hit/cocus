...
if dw_entry.event itemchanged( l_row, dw_entry.object.kyohi_kbn, string(dw_entry.object.kyohi_kbn[l_row]) ) <> 0 then
	return -1
end if
...
if i_syori_kbn <> "登録" then
		// 重複チェック
		if not of_dwchk_duplicate( dw_entry.object, l_user_id, l_kyohi_syubetu,l_kyohi_bumon ,l_rowid ) then
			return -1
		end if
	else
		// 重複チェック
		if not of_dwchk_duplicate( dw_entry.object, l_user_id, l_kyohi_syubetu,l_kyohi_bumon) then
			return -1
		end if
...
choose case dwo.name
	case "kyohi_syubetu", "kyohi_bumon","kyohi_kbn"
		l_input	= trim( data )
