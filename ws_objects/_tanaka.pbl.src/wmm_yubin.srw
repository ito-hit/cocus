...
choose case p_control
	case "callsection"
		...
		ddplb_data_kbn.enabled	= p_enabled
...
// ログ出力
of_add_db_log( "of_db_delete", 0, st_title.text + "[" + p_rec.yubin_no + " " + i_data_kbn + "]" + i_syori_kbn )
...
// ログ出力
of_add_db_log( "of_db_update", 0, st_title.text + "[" + p_rec.yubin_no + " " + i_data_kbn + "]" + i_syori_kbn )
...
// ログ出力
of_add_db_log( "of_db_insert", 0, st_title.text + "[" + p_rec.yubin_no + " " + i_data_kbn + "]" + i_syori_kbn )
...
choose case cocos.sqlcode
	case 0
		msg.of_error_dwo( dw_entry.object.yubin_no, fnc.strg.of_format( "重複する値が存在します。", "郵便番号" ) )
		dw_entry.setfocus()
		dw_entry.setcolumn( "yubin_no" )
...
l_file	= fnc.strg.of_format( "郵便番号登録_{1}_{2}", string( today(), "yyyymmdd" ), string( now(), "hhmmss" ) )
...
choose case dwo.name
	case "yubin_no", "todofu_mei", "sikutyo_mei","jis_code"
		l_input	= trim( data )
	...
	case "tyoiki_mei", "tyome", "jigyosyo_mei"
		l_input	= trim( data )
		l_colnm	= dwo.tag		// タグ値で項目名を取得
