$PBExportHeader$asv_commandbutton.sru
$PBExportComments$コマンドボタンコントロールの始祖オブジェクト
forward
global type asv_commandbutton from commandbutton
end type
end forward
...
event execution;
// ボタンがクリックされたときに処理するイベント //
if confirm_enabled then
	if not msg.of_question( fnc.strg.of_format( "{1}しますか？", fnc.strg.of_replace( fnc.strg.of_replace( this.text, "　", " " ), " ", "" ) ) ) then
		return -1
	end if
end if
...
on asv_commandbutton.create
end on

on asv_commandbutton.destroy
end on
...
choose case source.classname()
	case "p_drag_size_adjust"
		w_size_adjust.event callback( "commandbutton", this )
end choose
end event
