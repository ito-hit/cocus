$PBExportHeader$asv_datawindow.sru
$PBExportComments$データウインドウコントロールの始祖オブジェクト
forward
global type asv_datawindow from datawindow
end type
end forward
...
if of_get_key_enabled( key, keyflags ) then
	choose case key
		case KeyEnter!
			...
			// 垂直スクロールバーが設定されているカラムは「Ctrl+Enter」で改行する仕様とする（通常はフォーカス移動）
			if trim( this.describe( l_column + ".edit.vscrollbar" ) ) = "yes" then
				if keyflags = 2 then
					return 1
				end if
			end if
...
public function boolean of_get_key_enabled (keycode p_key, unsignedlong p_keyflags);// ファンクションキー
...
on asv_datawindow.create
end on

on asv_datawindow.destroy
end on
...
choose case source.classname()
	case "p_drag_size_adjust"
		w_size_adjust.event callback( "datawindow", this )
end choose
end event
