$PBExportHeader$asv_dropdownpicturelistbox.sru
$PBExportComments$ドロップダウンピクチャリストボックスコントロールの始祖オブジェクト
forward
global type asv_dropdownpicturelistbox from dropdownpicturelistbox
end type
end forward
...
event change pbm_cbneditchange
end type
global asv_dropdownpicturelistbox asv_dropdownpicturelistbox
...
public:
	...
	// 入力チェック(有効/無効)
	boolean	chk_required,chk_digits
	boolean	chk_type_num,chk_type_date,chk_type_time,chk_type_yearmonth
...
event type boolean input_check();return true
end event
...
public function boolean of_check ();return true
end function

on asv_dropdownpicturelistbox.create
end on

on asv_dropdownpicturelistbox.destroy
end on
...
choose case source.classname()
	case "p_drag_size_adjust"
		w_size_adjust.event callback( "dropdownpicturelistbox", this )
end choose
end event
