$PBExportHeader$asv_vprogressbar.sru
$PBExportComments$垂直プログレスバーコントロールの始祖オブジェクト
forward
global type asv_vprogressbar from vprogressbar
end type
end forward
...
global asv_vprogressbar asv_vprogressbar
...
public subroutine pgb_step (integer p_step)
...
public subroutine of_down (integer p_step);pgb_step( -1 * p_step )
end subroutine

public subroutine pgb_step (integer p_step);choose case true
	case this.minposition >= this.position + p_step
		this.position	= this.minposition
	case this.maxposition <= this.position + p_step
		this.position	= this.maxposition
	case else
		this.position	= this.position + p_step
end choose
end subroutine

public subroutine of_up (integer p_step);pgb_step( p_step )
end subroutine

public subroutine of_up ();of_up( i_step )
end subroutine

public subroutine of_init (integer p_min, integer p_max, integer p_now, integer p_step);i_min	= p_min
i_max	= p_max
i_now	= p_now
i_step	= p_step
end subroutine
...
on asv_vprogressbar.create
end on

on asv_vprogressbar.destroy
end on

event constructor;// コンストラクタ //

this.minposition	= i_min
this.setstep		= i_step
this.position		= i_now
...
choose case source.classname()
	case "p_drag_size_adjust"
		w_size_adjust.event callback( "vprogressbar", this )
end choose
end event
