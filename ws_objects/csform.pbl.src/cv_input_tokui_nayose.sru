$PBExportHeader$cv_input_tokui_nayose.sru
$PBExportComments$m_tokuiの名寄先の入力フォーム
forward
global type cv_input_tokui_nayose from cv_input
end type
end forward
...
global cv_input_tokui_nayose cv_input_tokui_nayose

type variables

public:
	str_mtokui		i_para_nayose[]			// データの受け渡しをする構造体
...
protected:
	str_mtokui		i_para_reset[]	// i_para_nayoseを初期化する用の変数
...
if upperbound( i_para_nayose ) = 0 then
	return 0
end if

return i_para_nayose[1].tokui_code
...
return i_para_nayose[1].bumon_code
end function

public function long of_get_nayose_code ();
// 外部からこの関数を呼んで、名寄せ先コードをreturnする関数
...
l_nayose_code	= long( l_input )

// まずは構造体にデータが格納されているかを確認 されていなければ入力値を返す
if upperbound( i_para_nayose ) = 0 then
	return l_nayose_code
end if

return i_para_nayose[1].nayose_code
end function
...
return i_para_nayose[1].seikyu_code
end function
...
return i_para_nayose[1].tanto_code
end function
...
return i_para_nayose[1].tokui_mei
end function
...
return i_para_nayose[1].tokui_mei_full
end function
...
return i_para_nayose[1].tokui_mei_syagai
end function
...
return i_para_nayose[1].tokui_mei_ryaku
end function
...
return i_para_nayose[1].tokui_mei_kana
end function
...
return i_para_nayose[1].yubin_no
end function
...
return i_para_nayose[1].torihiki_teisi_kbn
end function
...
return i_para_nayose[1].unso_code
end function
...
return i_para_nayose[1].todofuken_code
end function
...
return i_para_nayose[1].sikutyoson_code
end function
...
return i_para_nayose[1].syusoku_flg
end function
...
return i_para_nayose[1].syohizei_kbn
end function

on cv_input_tokui_nayose.create
call super::create
end on

on cv_input_tokui_nayose.destroy
call super::destroy
end on

event constructor;call super::constructor;
object_type	= "input_tokui_nayose"
...
if upperbound( l_mdl_tokui.ret ) > 0 then
	get												= true
	i_para_nayose									= i_para_reset	// マスタからデータ取得しているかもしれないので初期化する
	i_para_nayose[upperbound(i_para_nayose) + 1]	= l_mdl_tokui.ret[upperbound( l_mdl_tokui.ret )]
	
	// 入力値がない時もあるので、モーダルが取得した値をセット
	of_set_input( string( i_para_nayose[1].nayose_code ) )
...
l_ret			= true
l_input			= trim( of_get_input( ) )
i_para_nayose	= i_para_reset	// モーダルでデータ取得しているかもしれないので初期化する
l_item			= "名寄せ先コード"
...
if len( l_input ) > 0 then
	...
	choose case i_dbchk_mode
		case 0, 1
			// 存在チェックとついでにデータ取得
			if mst.of_tokui_getn( l_tokui_code, i_para_nayose ) > 0 then
				get	= true
...
type st_bikou from cv_input`st_bikou within cv_input_tokui_nayose
integer x = 1984
end type

type st_meisyo from cv_input`st_meisyo within cv_input_tokui_nayose
integer x = 768
integer width = 1200
end type

type cb_modal from cv_input`cb_modal within cv_input_tokui_nayose
integer x = 644
end type

type sle_input from cv_input`sle_input within cv_input_tokui_nayose
integer width = 300
end type

type st_title from cv_input`st_title within cv_input_tokui_nayose
string text = "名寄せ先"
end type
