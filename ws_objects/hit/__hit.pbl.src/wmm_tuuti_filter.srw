...
choose case cocos.sqlcode
	case 0
		msg.of_error_dwo( p_dwo, fnc.strg.of_format( "登録しようとしている通知フィルターはすでに登録があります。{1}【通知種別】:{2}{3}【送信対象】:{4}",code.crlf,l_db_kyohi_syubetu_mei,code.crlf,l_db_kyohi_bumon_mei))
		return false
	case 100
		return true
...
// 必須チェック
if len( string( dw_entry.object.kyohi_kbn[l_row] ) ) = 0 then
...
if  not uo_user.event sle_input_check( ) then
	if uo_user.sle_input.text="" then
		msg.of_error( "必須入力項目に値が設定されていません。" )
	end if
	uo_user.sle_input.setfocus()
	return -1
end if
