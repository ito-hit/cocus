...
public function boolean of_record_lock (string p_menu_id, string p_tab_id, boolean p_lock)
end prototypes
...
uo_menu.of_set_enabled( true )
uo_tab.of_set_enabled( true )

// 元の選択中だった処理区分を再セット(selectionchangedの再実行が必要)
//if not isnull( l_syori_kbn ) then
...
public function boolean of_record_lock (string p_menu_id, string p_tab_id, boolean p_lock);
if not lck.of_menu_lock( user.user_id, i_pg_id, p_menu_id, p_tab_id, p_lock ) then
...
i_menu_id	= uo_menu.of_get_input()
i_tab_id	= uo_tab.of_get_input()
...
choose case i_syori_kbn
	...
	case "変更", "削除"
		// 排他チェック
		if not of_record_lock( i_menu_id, i_tab_id, true ) then
			setredraw( true )
			return -1
		end if
		// 検索実行
...
//		if not dw_list.update( true, false ) = 1 then
		if dw_list.update( true, false ) <> 1 then
			...
			of_add_db_log( "update_event", cocos.last_sqlerrcode, cocos.last_sqlerrtext )
			// deleteバッファから戻す
			dw_list.rowsmove( 1, dw_list.deletedcount(), delete!, dw_list, 1, primary!)
			return -1
		end if
...
choose case i_syori_kbn
	case "変更"
		...
		if l_row > 0 then
			if not of_record_lock( i_menu_id, i_tab_id, false ) then
				return -1
			end if
		end if
