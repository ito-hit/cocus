...
// 処理区分変更後、即初期表示ボタンだとddplb_syori_kbn.selectionchangedが走らない？
// ddplb_syori_kbn.textだと選択した値が取得できる
...
if row > 0 then
	if dwo.name = "haiban_flg" then
		if this.object.haiban_flg[row] = 0 then
			this.object.haiban_flg[row] = 1
		else
			this.object.haiban_flg[row] = 0
		end if
	end if
end if
end event
