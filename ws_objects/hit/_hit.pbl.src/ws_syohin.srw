...
if rb_syoiro.checked then
	dw_list.dataobject	= "ds_syohin_iro"
elseif rb_sku.checked then
	dw_list.dataobject	= "ds_syohin_sku"
end if
...
// 商品CD
if uo_syohin.of_get_input( ) <> "" AND not uo_syohin.event sle_input_check( ) then
	return -1
end if

// 色番
// TODO:商品CDの入力値が無い場合エラーメッセージがでる
if uo_syohin.of_get_input_iro( ) <> "" AND not uo_syohin.event sle_iro_input_check( ) then
	return -1
end if
