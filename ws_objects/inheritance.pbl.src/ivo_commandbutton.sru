$PBExportHeader$ivo_commandbutton.sru
$PBExportComments$コマンドボタンコントロールの先祖オブジェクト
forward
global type ivo_commandbutton from asv_commandbutton
end type
end forward
...
global ivo_commandbutton ivo_commandbutton

type variables
protected:
	boolean	cmdb_retrieve, cmdb_update, cmdb_print, cmdb_close, cmdb_preview
...
on ivo_commandbutton.create
call super::create
end on

on ivo_commandbutton.destroy
call super::destroy
end on
...
event constructor;call super::constructor;object_type	= "commandbutton"
...
event clicked;call super::clicked;// 基本的にここにスクリプトは記載しない（executionイベントに書くように）
...
choose case true
	case cmdb_retrieve
		if not parent.dynamic of_evt_retrieve( i_wparam, i_lparam ) then
			return -1
		end if
	case cmdb_update
		if not parent.dynamic of_evt_update( i_wparam, i_lparam ) then
			return -1
		end if
	case cmdb_print
		if not parent.dynamic of_evt_print( i_wparam, i_lparam ) then
			return -1
		end if
	case cmdb_preview
		if not parent.dynamic of_evt_print_preview( i_wparam, i_lparam ) then
			return -1
		end if
	case cmdb_close
		close( parent )
...
// 基本的にここにスクリプトは記載しない（executionイベントに書くように）
return 0
end event
