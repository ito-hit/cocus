$PBExportHeader$ivo_tab.sru
$PBExportComments$タブコントロールの先祖オブジェクト~r~n~r~n※使い方~r~nいきなりWindowに貼り付けるのではなくて、単体で継承後、必要なタブページを作ってユーザーオブジェクトとして保存してからWindowに貼り付ける。もしくはそのまま貼り付けた後でタブページをユーザーオブジェクトで作成して追加する。
forward
global type ivo_tab from asv_tab
end type
end forward
...
public subroutine of_next_tab ()
public subroutine of_prior_tab ()
...
public subroutine of_next_tab ();
// 現在表示中タブの右タブを表示する（Ctrl+→での動作を想定）
integer	l_idx

l_idx	= this.selectedtab
	
if l_idx >= upperbound( this.control ) then
	l_idx	= 1
else
	l_idx	= l_idx + 1
end if
	
this.selecttab( l_idx )

end subroutine

public subroutine of_prior_tab ();
// 現在表示中タブの左タブを表示する（Ctrl+←での動作を想定）
integer	l_idx

l_idx	= this.selectedtab
	
if l_idx = 1 then
	l_idx	= upperbound( this.control )
else
	l_idx	= l_idx - 1
end if
...
event constructor;call super::constructor;object_type	= "tab"
