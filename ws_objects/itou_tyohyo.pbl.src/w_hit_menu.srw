...
type cb_64 from ivo_commandbutton within w_hit_menu
end type
type cb_65 from ivo_commandbutton within w_hit_menu
end type
end forward
...
this.Control[iCurrent+64]=this.cb_64
this.Control[iCurrent+65]=this.cb_65
end on
...
integer taborder = 70
boolean bringtotop = true
string text = "キープ期限警告リスト発行"
boolean enter2tab = false
end type

event execution;call super::execution;
winctl.of_open( "wp_keep_warning" )
end event
...
integer taborder = 80
boolean bringtotop = true
string text = "加工状況照会(サブ)"
boolean enter2tab = false
end type

event execution;call super::execution;
winctl.of_open( "ws_kako_koutei" )
end event
